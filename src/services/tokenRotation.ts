...
function performRotationSteps(deps: TokenRotationDeps): void {
  ...
  // 「2分もブロックするならsleepの間だけ解放しよう」としないこと。解放するとsleepの間に
  // 2つ目のローテーションが開始でき、後発のPUT（M入りURL）と先発の刈り取り
  // （プロパティを"N"に）が交錯して、LINEはMを向いているのにGASはNしか受け付けない
  // 全停止を作れる。ローテーションが同時に2つ走らないことがこのロックの存在理由である。
