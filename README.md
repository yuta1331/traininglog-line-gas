...
⚠️ **手順4は省略しないでください。** 「GASが`t`を`e.parameter`から受け取れる」というプラットフォームの挙動そのものはテスト環境で実測済みですが、ここで確認するのはそれではなく、**このデプロイの設定が揃っているか**です。スクリプトプロパティとWebhook URLの`t`の食い違い、URLの打ち間違い、デプロイ版の更新漏れは、どれもfail-closedにより「全メッセージが黙って処理されない」として現れます。疎通確認を省略するとそれに気づけません。
...
**トリガーの通知設定を確認してください。** ローテーションが失敗すると `rotateWebhookToken` が例外で終了し、GASがオーナー宛に失敗通知メールを送ります。**これが唯一の検知経路です**（LINEのpush通知は採用していません。通知経路自体がローテーション失敗と同じ依存＝LINE APIを持ってしまうためです）。

⚠️ **`setupRotationTrigger()` が作ったトリガーの通知設定は既定値のままです。** 通知設定はプログラムから指定できないため、実行後にApps Scriptエディタの「トリガー」画面で該当トリガーを編集し、手で変更する必要があります。既定は「日次ダイジェスト」の場合があり、その場合メールは翌日にまとまって届きます。「今すぐ通知を受け取る」への変更を推奨します（必須ではありません。失敗時に残る`"C,N"`は構造上安全でBotは動き続けるため、通知が遅れてもローテーションの衛生の問題であり、可用性の問題にはなりません）。

⚠️ **失敗通知メールが送られるのはトリガーによる実行だけです。** エディタから `rotateWebhookToken` を手動実行して失敗した場合、エラーは「実行数」画面に出るだけでメールは届きません（実測で確認）。通知経路そのものを検証したいときは、Webhookの利用をOFF（`active:false`）にしたうえで**分ベースのタイマートリガーを一時的に作って**失敗させてください。確認後、そのトリガーの削除を忘れないこと（`setupRotationTrigger()` を実行し直せば同じハンドラのトリガーを消して月次のものを作り直すので、それでも片付きます）。
